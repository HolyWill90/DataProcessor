"""
Provider Management UI for Financial Data Harmonizer.
This Streamlit-based UI allows creating and managing provider configurations.
"""
import os
import sys
import json
...
try:
    ...
    st.error(f"Error importing ProviderConfig: {e}")
    provider_config = None

# Set page config
st.set_page_config(
    page_title="Provider Manager - Financial Data Harmonizer",
    page_icon="🛠️",
    layout="wide",
...
if "provider_config" not in st.session_state:
    try:
        ...
        st.error(f"Failed to initialize ProviderConfig: {e}")
        st.session_state.provider_config = None

if "providers" not in st.session_state:
    if st.session_state.provider_config:
        st.session_state.providers = list(st.session_state.provider_config.providers_cache.keys())
    else:
...
def load_provider(provider_name):
    ...
    if not st.session_state.provider_config:
        st.error("Provider configuration manager not available")
        return get_default_provider_config()
    
    try:
        provider_data = st.session_state.provider_config.get_provider_settings(provider_name)
...
def save_provider(provider_data):
    ...
    try:
        ...
        st.session_state.provider_config.save_provider(provider_data)
        
        # Update providers list
        if provider_name.upper() not in st.session_state.providers:
            st.session_state.providers.append(provider_name.upper())
...
def provider_selector():
    ...
    st.sidebar.title("Provider Manager")
    ...
    if action == "Create New":
        ...
        selected = st.sidebar.selectbox(
            ...
            index=st.session_state.providers.index(st.session_state.selected_provider) if st.session_state.selected_provider in st.session_state.providers else 0
        )
...
def main():
    ...
    # Delete provider option (only in edit mode)
    if st.session_state.edit_mode and st.session_state.selected_provider:
        st.write("---")
        st.subheader("Danger Zone")
        
        if st.button("Delete Provider", type="primary", use_container_width=True):
            if st.session_state.provider_config and st.session_state.selected_provider:
                try:
                    provider_path = os.path.join(
                        st.session_state.provider_config.config_dir, 
                        f"{st.session_state.selected_provider}.json"
                    )
                    ...
                    if st.session_state.selected_provider in st.session_state.providers:
                        st.session_state.providers.remove(st.session_state.selected_provider)
                    
                    st.success(f"Provider {st.session_state.selected_provider} deleted")
                    reset_form()
                    st.experimental_rerun()
                except Exception as e:
                    st.error(f"Failed to delete provider: {str(e)}")
